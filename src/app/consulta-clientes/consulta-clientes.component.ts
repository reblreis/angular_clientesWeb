...
export class ConsultaClientesComponent implements OnInit {
  ...
  ngOnInit(): void {  
    ...
    //ler o token do usuário gravado na local storage
    const token = localStorage.getItem(config.authToken) as string;
    //adicionando o token no cabeçalho da requisição HTTP
    const httpHeaders = new HttpHeaders({
      Authorization: 'Bearer ' + token
    });
   
    //fazendo uma requisição para consultar os clientes na API
    this.httpClient.get(config.apiUrlClientes + '/clientes', { headers: httpHeaders })
  ...
  onDelete(id: string) : void {
    ...
    if(confirm('Deseja realmente excluir o cliente selecionado?')) {
      ...
      this.httpClient.delete(config.apiUrlClientes + "/clientes/" + id, { responseType: 'text' })
        .subscribe({
          next: (data) => {
